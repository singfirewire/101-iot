...
const char* topic_remote = "iot/timer-relay/remote";  // เพิ่ม topic สำหรับรีโมท
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    // จัดการคำสั่งจากรีโมท
    else if (topicStr == topic_remote) {
        String command = doc["command"];
...
bool reconnectMQTT() {
    if (mqtt.connect(device_id, mqtt_user, mqtt_password)) {
        ...
        mqtt.subscribe(topic_remote);  // เพิ่ม subscribe topic ของ remote
        publishStatus();
        return true;
    }
    return false;
